...
import 'package:onproperty/utils/flushbar_mixin.dart';
import 'package:onproperty/widgets/menu_widget.dart';
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton<Choice>(
            ...
            itemBuilder: (context) {
              return [
                Choice(icon: Icons.info_outline, title: "User Profile"),
                Choice(icon: Icons.block, title: "Block")
              ].map((Choice choice) {
...
class ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> onSendMessage(String content, int type) async {
    ...
    if (content.trim() != '') {
      ...
      await FirebaseFirestore.instance
        ...
        "mainId": widget.userId,
        "username": FirebaseAuth.instance.currentUser.displayName,
        'house_id': widget.houseId,
        "photo":
            'https://www.vectorstock.com/royalty-free-vector/businessman-character-avatar-isolated-vector-12613615',
        "time": DateTime.now(),
      });
      await FirebaseFirestore.instance
          .collection("Chat_Col")
  ...
  Future<void> onSendreply(String content, int type, String id) async {
    // type: 0 = text, 1 = image, 2 = sticker
    if (content.trim() != '') {
      textEditingController.clear();
      var documentReference = FirebaseFirestore.instance
                                                                        ...
                                                                        builder: (context) =>
                                                                            AlertDialog(
                                                                              ...
                                                                              actions: <Widget>[
                                                                                FlatButton(
                                                                                  onPressed: () async {
                                                                                    await FirebaseFirestore.instance.collection("Pchat").doc("${widget.peerId}").collection("members").doc(snapshot.data.docs.single["user_id"]).update({
                                                                                      "pending": false,
                                                                                    });
                                                                        ...
                                                                        builder: (context) =>
                                                                            AlertDialog(
                                                                              ...
                                                                              actions: <Widget>[
                                                                                FlatButton(
                                                                                  onPressed: () async {
                                                                                    await FirebaseFirestore.instance.collection("Pchat").doc("${widget.peerId}").collection("members").doc(snapshot.data.docs.single["user_id"]).delete();
